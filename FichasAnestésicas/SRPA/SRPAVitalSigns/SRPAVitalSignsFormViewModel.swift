...
@MainActor
final class SRPAVitalSignsFormViewModel: ObservableObject {
    // MARK: - Dependencies
    let repo: SRPAVitalSignsEntryRepository
    let srpa: SRPA
    let user: User
    let context: ModelContext
    
    // MARK: - Editing state
    @Published var existingEntry: SRPAVitalSignEntry?
    ...
    @Published var srpaStart: Date?
    @Published var srpaEnd: Date?
    ...
    init(
        repo: SRPAVitalSignsEntryRepository,
        srpa: SRPA,
        user: User,
        context: ModelContext,
        existingEntry: SRPAVitalSignEntry? = nil
    ) {
        self.repo = repo
        self.srpa = srpa
        self.user = user
        ...
        self.anesthesiaStart = srpa.surgery.anesthesia?.start
        self.anesthesiaEnd = srpa.surgery.anesthesia?.end
        self.surgeryStart = srpa.surgery.start
        self.surgeryEnd = srpa.surgery.end
        self.techniques = srpa.surgery.shared?.techniques ?? []
        self.asaClassification = srpa.surgery.shared?.asa
        self.anesthesiaPosition = srpa.surgery.anesthesia?.position ?? []
        
        // Calcular idade do paciente usando AgeContext
        let ageContext = AgeContext.inSurgery(srpa.surgery)
        self.patientAge = ageContext.ageInYears(from: srpa.surgery.patient.birthDate)
        
        if let entry = existingEntry {
            // Modo edição
            // Garante que usamos a instância gerenciada pelo mesmo ModelContext deste ViewModel
            let managedEntry: SRPAVitalSignEntry
            let id = entry.srpaVitalSignsId
            if let fetched = try? context.fetch(
                FetchDescriptor<SRPAVitalSignEntry>(
    ...
    func makeEntry(from id: String? = nil) -> SRPAVitalSignEntry {
        // Build according to @Model VitalSignEntry initializer
        let entry = SRPAVitalSignEntry(
            srpaVitalSignsId: id ?? UUID().uuidString,
            srpa: srpa,
            timestamp: timestamp
        )
    ...
    func getAll() -> [SRPAVitalSignEntry] {
        return repo.getAll(for: srpa)
    }
    ...
    func updateCurrent() throws {
        ...
        try repo.update(current, for: srpa, by: user)
    ...
    func deleteCurrent() throws {
        ...
        try repo.delete(current, for: srpa, by: user)
