...
struct SRPASheetMedicationsView: View {
    var srpa: SRPA

    private var combinedMedicationsText: Text {
        
        func itemText(_ m: SRPAMedicationEntry) -> String {
            let time = m.timestamp.formatted(date: .omitted, time: .shortened)
            return "\(m.name) \(m.dose) (\(time))"
        }

        var bucketVR: [SRPAMedicationEntry] = []
        var bucketCristaloide: [SRPAMedicationEntry] = []
        var bucketHemoderivados: [SRPAMedicationEntry] = []
        var bucketEV: [SRPAMedicationEntry] = []
        var otherByRoute: [AdministrationRoute: [SRPAMedicationEntry]] = [:]
        var otherByRouteOrder: [AdministrationRoute] = [] // preserve first-seen order

        for m in srpa.medications.sorted(by: { $0.timestamp < $1.timestamp }) {
        ...
        func makeSegment(title: String, items: [SRPAMedicationEntry]) -> Text? {
